  ...
  try {
    ...
    if (!existing) {
      await chrome.offscreen.createDocument({
        url: "offscreen.html",
        reasons: ["BLOBS"],
        justification: "Keep local bridge polling active",
      });
    }
  } catch {}
}
...
armKeepaliveAlarm();
console.log("[pi-chrome] Service worker started, triggering pollLoop");
void pollLoop();
...
async function pollLoop() {
  ...
  try {
    while (true) {
      const clientName = `${CLIENT_NAME} [${lastDebug}]`;
      const response = await fetch(`${BRIDGE_URL}/next?name=${encodeURIComponent(clientName)}`, {
        cache: "no-store",
      });
    ...
    console.error("[pi-chrome] pollLoop error:", error);
...
async function handleCommand(command) {
  ...
  try {
    ...
    lastDebug = `disp_ok:${command.action}`;
    await postResult({ id: command.id, ok: true, result });
    lastDebug = `post_ok:${command.action}`;
  } catch (error) {
    lastDebug = `err:${command.action}:${error?.message ?? String(error)}`;
...
async function postResult(result) {
  ...
  try {
    if (json.length < 30000) {
      const res = await fetch(`${BRIDGE_URL}/result?data=${encodeURIComponent(json)}`, {
        cache: "no-store",
      });
      lastDebug += `:res_get${res.status}`;
      return;
    }
  } catch (err) {
    lastDebug += `:get_fail:${err?.message ?? String(err)}`;
  }
  try {
    const res = await fetch(`${BRIDGE_URL}/result`, {
      method: "POST",
      headers: { "content-type": "text/plain" },
      body: json,
    });
    lastDebug += `:res_post${res.status}`;
  } catch (err) {
    lastDebug += `:post_fail:${err?.message ?? String(err)}`;
  }
}
