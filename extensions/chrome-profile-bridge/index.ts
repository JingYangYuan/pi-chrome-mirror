...
function corsHeadersFor(request: IncomingMessage): Record<string, string> {
	...
	return {
		"access-control-allow-origin": isExt ? origin : "*",
...
class ChromeProfileBridge {
	...
	private async handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
		...
		try {
			appendFileSync("/tmp/pi-chrome-requests.log", JSON.stringify({ time: new Date().toISOString(), method: request.method, url: request.url, headers: request.headers }) + "\n");
		} catch {}
		...
		if ((request.method === "POST" || request.method === "GET") && url.pathname === "/result") {
			if (!isBrowserOriginAllowed(request)) {
				sendJson(response, 403, { ok: false, error: "browser origin not allowed" });
				return;
			}
			...
			try {
				if (request.method === "GET") {
					...
					if (!dataParam) {
						sendJson(response, 400, { ok: false, error: "missing data param" }, corsHeaders);
						return;
					}
					result = JSON.parse(dataParam) as BridgeResult;
				} else {
					result = JSON.parse(await readRequestBody(request)) as BridgeResult;
				}
			} catch (err) {
				sendJson(response, 400, { ok: false, error: "invalid json" }, corsHeaders);
				return;
			}
...
export default function (pi: ExtensionAPI): void {
	...
	const globalState = globalThis as typeof globalThis & {
		[PI_CHROME_GLOBAL_KEY]?: { version: string; root: string; token?: symbol; bridge?: ChromeProfileBridge };
	...
	if (alreadyLoaded?.bridge) {
		try { alreadyLoaded.bridge.stop(); } catch {}
	}
	...
	globalState[PI_CHROME_GLOBAL_KEY] = { version: PI_CHROME_VERSION, root: currentRoot, token: instanceToken, bridge };
